...
for(i in 1:length(models)){
  ...
  # remove hurdle fixed effects 
  q3 <- q3[which(!(substr(rownames(q3),3,4)=="hu")),]
  
  # remove second lot of hurdle fixed effects 
  q3 <- q3[which(!(substr(rownames(q3),4,5)=="hu")),]
...
lineadj <- rev(seq(-0.35,0.35, length.out=length(models)))  # here you can control the gap between points
# keep the two numbers identical (order than sign) to create evenly spaced groups
# smaller numbers which create closer points and large will create more spread out points 

par(mar=c(5,10,1,1))# this controls the size of margins required to show the labels
par(mfrow=c(1,1))

plot(NA, pch=16, xlab="Scaled fixed effects estimates",  yaxt='n', ylab='',
     xlim=range(allcis[,1:5]),
     ylim=range(1:length(unique(allcis$var))) + range(lineadj))
